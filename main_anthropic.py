...
def run_agent(manual_mode: bool = False) -> None:
    ...
    while True:
        # Retry with backoff on rate limit and overloaded errors
        for attempt in range(5):
            try:
                ...
                if e.status_code == 529:
                    wait = 30 * (attempt + 1)
                    console.print(f"[yellow]⚠ Anthropic overloaded — waiting {wait}s before retrying...[/yellow]")
                    import time
                    time.sleep(wait)
                else:
                    raise
